...
{
  options.home-manager.window-manager.x11.picom.enable = lib.mkEnableOption "picom config" // {
    default = config.home-manager.window-manager.x11.enable;
  };

  config = lib.mkIf config.home-manager.window-manager.x11.picom.enable {
    services.picom = {
      enable = true;
      backend = if config.home-manager.window-manager.x11.nvidia.enable then "glx" else "egl";
      fade = true;
      fadeDelta = 2;
      vSync = true;
      settings = {
